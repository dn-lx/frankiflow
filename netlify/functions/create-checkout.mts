...
function stripeBody(params){const body=new URLSearchParams();for(const [k,v] of Object.entries(params)){if(v!==undefined&&v!==null&&v!=="")body.append(k,String(v))}return body}
export default async (req)=>{
  if(req.method!=="POST")return json({error:"Method not allowed"},405);
  const token=(req.headers.get("authorization")||"").replace(/^Bearer\s+/i,""); if(!token)return json({error:"Unauthorized"},401);
  let admin; try{admin=await getAdminFromToken(token)}catch(e){return json({error:e.message},500)} if(!admin)return json({error:"Admin access required"},403);
  let input;try{input=await req.json()}catch{return json({error:"Invalid JSON"},400)}
  const amount=Math.round(Number(input.amount_cents)); const email=String(input.customer_email||"").trim(); const description=String(input.description||"").trim();
  ...
  const insert=await supabaseRequest('/rest/v1/frankiflow_payments',{method:'POST',body:JSON.stringify({quote_request_id:input.quote_request_id||null,description,amount_cents:amount,currency:'eur',customer_email:email,status:'created',created_by:admin.id,metadata:{source:'frankiflow-admin'}})}); if(!insert.ok)return json({error:'Could not create payment record',detail:await insert.text()},500); const [payment]=await insert.json();
  const siteUrl=(env('SITE_URL')||new URL(req.url).origin).replace(/\/$/,'');
  const params={mode:'payment',success_url:`${siteUrl}/?payment=success`,cancel_url:`${siteUrl}/?payment=cancelled`,customer_email:email,client_reference_id:payment.id,'line_items[0][price_data][currency]':'eur','line_items[0][price_data][unit_amount]':amount,'line_items[0][price_data][product_data][name]':description,'line_items[0][quantity]':1,'metadata[frankiflow_payment_id]':payment.id,'metadata[created_by]':admin.id};
  ...
  return json({payment_id:payment.id,session_id:stripe.id,url:stripe.url});
};

...
async function downloadQuotePdf(){
  ...
  try{
    ...
    doc.setFillColor(11,45,75);doc.rect(0,0,pageWidth*.58,3,'F');doc.setFillColor(27,153,152);doc.rect(pageWidth*.58,0,pageWidth*.24,3,'F');doc.setFillColor(111,207,131);doc.rect(pageWidth*.82,0,pageWidth*.18,3,'F');
    try{const logo=await imageDataUrl('/assets/frankiflow-logo.png');doc.addImage(logo,'PNG',16,8,36,22,undefined,'FAST');}catch(error){console.warn('PDF logo unavailable',error);}
    ...
    doc.setFont('helvetica','normal');doc.setTextColor(128,144,155);doc.setFontSize(7.5);doc.text(`${money(calc.monthly)} / ${lang==='de'?'Monat':'month'} · ${vatNote}`,16,71);

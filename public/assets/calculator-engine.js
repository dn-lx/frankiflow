...
export function calculatePricing(cfg,s){
  ...
  // Round customer-visible components first so every displayed sum reconciles to the cent.
  const floorVisit=gross(floorVisitNet);
  const equipmentVisit=gross(equipmentVisitNet);
  const windowCharge=gross(windowChargeNet);

  // Window cleaning selected together with a recurring cleaning service is performed and charged
  // on every selected cleaning visit. Therefore it must increase both "price per visit" and the
  // corresponding monthly total. Window-only cleaning follows the same per-visit rule.
  const visitTotal=s.windowOnly
    ?windowCharge
    :round2(floorVisit+equipmentVisit+(s.windows?windowCharge:0));
  const monthly=round2(visitTotal*visits);
  ...
  const firstFloor=gross(discountedFloorNet);
  const firstEquipment=gross(discountedEquipmentNet);
  const firstWindow=gross(discountedWindowNet);
  const firstVisit=s.windowOnly
    ?firstWindow
    :round2(firstFloor+firstEquipment+(s.windows?firstWindow:0));
  const firstMonth=round2(firstVisit*visits);
  ...
  return {...s,service,reductionPct,windowReductionPct,visits,vatRate,promoPct,
    ...
    floorVisit,equipmentVisit,windowCharge,visitTotal,subtotal:monthly,monthly,firstVisit,firstMonth};
}
